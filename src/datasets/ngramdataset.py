...
class NGramDataset(Dataset):
    ...
    '''
      ...
      '''
    @classmethod
    def from_json_files(cls, input_path: str, context: Context, feature_names: list, label_name: str, limit: int = -1) -> Dataset:
    ...
    def __getitem__(self, item: int) -> (torch.Tensor, torch.Tensor):
        try:
            ...
            IOUtil.log_error(f'Index {item} out of range {str(e)}')
            return None
    ...
    def collate_batch(batch: list) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        ...
        # contexts = lambda t: [item for sublist in t for item in sublist]
