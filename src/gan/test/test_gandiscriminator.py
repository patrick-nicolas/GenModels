...
class TestGANDiscriminator(TestCase):
    def test_build_from_conv(self):
        try:
            ...
            disc = GANDiscriminator.build_from_conv('model-1', dim, z_dim, hidden_dim, out_dim, params)
            IOUtil.log_info(repr(disc))
    ...
    def test_build_from_dff(self):
        try:
            ...
            disc = GANDiscriminator.build_from_dff(model_id, input_size, hidden_dim, output_size, dff_params)
            IOUtil.log_info(repr(disc))
        except Exception as e:
            self.fail(str(e))
